...
  detalleVentasOriginal: detalleVenta[] = []; // Se usa para la comparación

  // ⭐ NUEVAS LISTAS para almacenar los cambios
...
  // --- Variables de la lógica de venta/modificación ---
...
  // --- Métodos de la funcionalidad principal ---
...
        peticionesDetalles.push(this.detalleVentaS.delete(detalle.idDetalleVenta));
      }
    });
...
        },
        error: (error) => {
          console.error('❌ Error al procesar los detalles de venta:', error);
...
      console.log('➡️ No hay cambios en los detalles de venta. Pasando al siguiente paso.');
      this.actualizarClienteYVentaPrincipal();
    }
  }
...
        this.aplicarFiltro();
      }
      this.mostrarModal(productoEnStock.producto.nombre, productoEnStock.cantidad);
      return;
    }
...
        subtotal: cantidad * producto.producto.precio,
      });
    }
