...
console.log(`Usuario actual: ${this.username}`); // Verifica el usuario actual
    this.carritoService.listarProductosDeUsuario(this.username).subscribe({
      next: (response: ApiResponse) => {
        if (response.success) {
          console.log('Respuesta exitosa del backend (datos crudos):', response.data);

          const rawDetalles: RawDetalleCarritoProducto[] = response.data as RawDetalleCarritoProducto[];

          // Mapea y transforma los datos, asegurando que los campos monetarios sean 'string'
          this.detallesCarrito = rawDetalles.map((item: RawDetalleCarritoProducto) => {
            return {
              ...item,
          ...
          console.log('Productos del carrito (transformados a string para visualización):', this.detallesCarrito);
  ...
  eliminarProducto(idDetalleCarrito: number): void {
    console.log(`Intentando eliminar producto con ID: ${idDetalleCarrito} del carrito del usuario: ${this.username}`);
    if (confirm('¿Estás seguro de que quieres eliminar este producto del carrito?')) {
      this.carritoService.eliminarProductoDeCarrito(this.username, idDetalleCarrito).subscribe({
        next: (response: ApiResponse) => {
          if (response.success) {
